	...
	<?php endif; ?>
		...
		while ( $meteor_loop->have_posts() ) : $meteor_loop->the_post(); ?>
			...
			<div class="mslide mslide-<?php echo $meteor_count; ?>">
				...
				$slideURL = get_post_meta( $post->ID, "slide_url_value", $single = true );
					
				if ( $slideURL != "" ): 

						//Determine if link is a YouTube Video
    					if( preg_match("#^https?://(?:www\.)?youtube.com#", $slideURL)){

						
							echo '<a href="' . $slideURL . '&width=100%&height=100%" title="' . get_the_title() . '" rel="wp-video-lightbox">' .  get_the_post_thumbnail( $post->ID, 'featured-slide' ) . '</a>';
						}
						else{

							echo '<a href="' . $slideURL . '" title="' . get_the_title() . '">' . get_the_post_thumbnail( $post->ID, 'featured-slide' ) . '</a>';
						}
