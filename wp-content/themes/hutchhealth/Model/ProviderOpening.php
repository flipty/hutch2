...
class ProviderOpening extends Model{
	...
	function get_active_openings( $query ) {
		if ( $query->is_main_query() && $query->is_post_type_archive( 'provider-opening' ) && !is_admin() ) {
	...
	function hook_canonical() {
	    ...
	    echo '<link rel="canonical" href="' .get_bloginfo('url'). '/provider-openings/" />';
	}
